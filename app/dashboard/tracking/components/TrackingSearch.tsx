...
import useThemeSettingsStore, { ThemeColor } from '@/store/themeSettingsStore';
...
const TrackingSearch: React.FC = () => {
  ...
  // Obtener el color del tema desde el store, asegurando el tipo correcto
  const themeColor = useThemeSettingsStore(state => state.themeColor as ThemeColor | 'default');
  ...
  const operatorData = useMemo(() => {
    ...
    if (loggedOperator.branch_id) {
      ...
      } else if (typeof loggedOperator.branchReference === 'object' && 
                 loggedOperator.branchReference !== null && 
                 'id' in loggedOperator.branchReference) { 
        branchId = (loggedOperator.branchReference as any).id; // Remove '|' and cast to any
        branchPath = `branches/${branchId}`;
      }
    }
  ...
  const themeColorString = useMemo(() => 
    ...
    // @ts-ignore - // Linter doesn't see the full state type including "default"
    themeColor === 'default' ? 'purple' : themeColor, 
  [themeColor]);
        ...
        packages.forEach((pkg, idx) => {
          ...
          if (pkg.client && pkg.client.shipping_insurance !== undefined) {
            ...
            if (typeof pkg.client.shipping_insurance === 'string') {
              const insuranceStr: string = pkg.client.shipping_insurance; // Assign to typed variable
              const insuranceValue = insuranceStr.toLowerCase();
  ...
  const updateWithBasicClientInfo = () => {
    ...
    if (client && searchResults.length > 0) {
      ...
      const basicClient: Client = {
        ...
        shipping_insurance: client.shipping_insurance === true,
    ...
    try {
      ...
      if ('shipping_insurance' in client) {
        if (typeof client.shipping_insurance === 'boolean') {
          ...
          const insuranceStr: string = client.shipping_insurance; // Assign to typed variable
          hasShippingInsurance = insuranceStr === 'true' || 
                              insuranceStr === '1' || 
                              insuranceStr.toLowerCase() === 'yes';
  ...
  const loadBranchStats = useCallback(async () => {
    ...
    try {
      ...
      // Check typeof string before splitting
      if (typeof operatorData.branchReference === 'string') {
        console.log('🔍 branchReference es string:', operatorData.branchReference);
        try {
          // Assertion to string before split
          const parts = (operatorData.branchReference as string).split('/'); 
          branchId = parts[parts.length - 1];
                          ...
                          key={result.id || result.trackingNumber}
                            ...
                            >
                              <td className="p-3">
                                ...
                                {(result as any).origin && (result as any).destination && (
            ...
            <div className="max-h-[250px] overflow-y-auto pr-1 -mr-1 space-y-1.5">
              {filteredClients.length === 0 ? (
                ...
                filteredClients.map((client, index) => {
                  ...
                  const clientId = client.id || `client-${index}-${client.name.replace(/\s+/g, '-').toLowerCase()}`;
