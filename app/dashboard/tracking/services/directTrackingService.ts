    ...
    if (/^[A-Za-z0-9]{6,}$/.test(query.trim())) {
      ...
      if (result.success && result.data) {
        ...
        const processedData = dataArray.map(item => {
          ...
          // Cast item to any to check for the 'user' property
          if ((item as any).user && !item.client) {
            console.log('🔄 DirectTrackingService.searchTrackings - Transformando campo "user" a "client" para objeto:', {
              id: item.id,
              trackingNumber: item.trackingNumber
            });
            
            // Access user properties via the 'any' cast
            ...
            return {
              ...
              client: {
                ...
                shipping_insurance: user.shipping_insurance
              }
            };
          }
